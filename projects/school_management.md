...
Use Java or C# to design and implement a desktop application for School Management. The application should have two types of users (the teacher and the administrator user) which have to provide a username and a password in order to use the application.

The teacher can perform the following operations:
* Search the list of students
* Give marks on his class to students

The administrator can perform the following operations:
* CRUD on students
* CRUD on teachers information (including the class association - e.g., maths, english, computer science)
* Generate two types of report files, one in pdf format and one in txt or html format, with the student marks. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The students can also login in the application, and see the list of their marks. Their accounts are created by the administrator.
* The students get a live notification when a new mark is added to their profile.
* The students can rate once the teachers (between 1-5 stars). Everyone can see the accumulated rating.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the students & classes controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Students can rate the classes and teachers (between 1-5 stars) and see their current ratings average. The admin user can edit the ratings.
* Students can request to the admin to be transferred to another class or teacher on a particular class. The admin can see the requested and approve/reject them, which will update the data accordingly.
