...
Use Java or C# to design and implement a desktop application for Building management. The application should have multiple types of users: tenant, administrator and maintenance man,  which have to provide a username and a password in order to use the application.

The tenant perform the following operations:
* Send current month's consumption for the different utilities
* View the list of bills to pay (but can't pay yet)

The administrator can perform the following operations:
* CRUD on regular users' information.
* Create the bills to pay for the user (for start, something simple like name & amount)
* Generate two types of report files, one in pdf format and one in txt or html format, with the consumption per building he manages. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The tenant can request help from maintenance, for this a request is sent to the maintenance team. Every maintenance man that is online will get a live notification for the request, which can be accepted/rejected, so the tenant will know if somebody will help.
* The maintenance will also generate a bill for the provided services
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the tenants and bills controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* The tenant can pay the bills from his own virtual wallet. The administrator can update the wallet's current amount.
* The tenant can view statistics with the bills over a given period of months, and the consumptions
* If the tenant doesn't pay the bills by the deadline, they will accumulate a penalty, which will be shown when paying that particular bill.
