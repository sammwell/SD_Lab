...
Use Java or C# to design and implement a desktop application for Movie Rental. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Search movies by genre, title, actors, etc
* Borrow movies for a specific period and return them

The administrator can perform the following operations:
* CRUD on movies
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the current movies and their history (borrowed or not) The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* An user can put himself in the queue for a movie when it's returned. When that happens, the user gets a live notification.
* If the user returns the movie later than the expected return date, a penalty will be accumulated on his account.
* Users can add/remove movies from their favourites list.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the movies & borrowings controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Users can rate (between 1-5 stars) and review (textual description) the movies, those reviews will be public. They can also search by the rating. The admin user can edit the ratings.
* If a user receives 5 penalty points, he won't be able to borrow another movie for another 1 month. The penalty resets after that time.
* Administrator can change user's subscription which limits the number of movies he can borrow at a time. The subscriptions can be added/changed by the admin
