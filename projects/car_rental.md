...
Use Java or C# to design and implement a desktop application for Car Rental. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Search list of available cars to rent (name, type, power, etc)
* Rent a single car at a time, for a specified duration

The administrator can perform the following operations:
* CRUD on available cars
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the cars and their renting history. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* An user can put himself in the queue for a cars when it's returned. When that happens, the user gets a live notification.
* If the user returns the car later than the expected return date and/or in a worse state, a penalty will be accumulated on his account.
* The user will upload a picture with the car when returning it.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the car-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Regular Users can rate once the cars (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
* Regular users can add/remove cars from their favourites list
* Administrator can change user's subscription which limits the number of cars he can rent in one week and the duration for which he can do it. The subscriptions can be added/changed by the admin
* After each rent is returned/completed, the user can leave a review which is visible for all users that look at that car.
