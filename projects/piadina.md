...
Use Java or C# to design and implement a desktop application for a Piadina shop. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Search piadinas by different components, price, size
* Order a piadina from the list, with money from his virtual wallet (the admin sets its value for now)

The administrator can perform the following operations:
* CRUD on piadina. Each is made up of several items that can also be CRUD-ed by the admin
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the sold piadinas and their configurations. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The ordering process takes some time (virtual delay). The user gets a live notification when the order is completed. This is done by the admin switching the status from initially "Ordered" to "In Progress" and then "Delivered".
* Users can add/remove piadinas to/from their favourites list
* Users can rate once the piadinas (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the piadinas controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Users can order a custom created piadina, with the available items.
* Users can now write reviews on the ordered piadinas, and all other users will be able to view them.
* Users can order at a time multiple and different types of piadinas, by adding them to the cart and checking them out at the end. The progress done earlier for a single piadina will be progressive based on the entire order's status
