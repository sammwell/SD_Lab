...
Use Java or C# to design and implement a mobile and desktop application for Urban Public Transport. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* View bus timetables
* Purchase a ticket for a specific route for 30 mins, 1 hour or 1 day, with money from his virtual wallet (the admin sets its value for now)

The administrator can perform the following operations:
* CRUD on bus routes (book information: title, author, genre, quantity, and price).
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the routes & purchased tickets for each. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The user can request a ticket subscription for 1 week or 1 month. This needs to be approved by the admin, that validates the entered details.
* The user gets a live notification when his subscription request was validated.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the tickets & timetables controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Regular Users can rate once the routes (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
* Regular users can add/remove routes from their favourites list
* When the timetable changes, the user gets a notification if that route is on his favourites list.
