...
Use Java or C# to design and implement a desktop application for Cookie Shop. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Search cookies (where cookie is any kind of sweet product) by properties (type, price, quantity of sweeteners)
...
* Generate two types of report files, one in pdf format and one in txt or html format, with the cookies bought in the last X days - as selected by the admin when generating. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* Users can add/remove cookies to/from their favourites list
* Users can rate once the cookies (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
* An user gets a live notification when a favourite cookie is back on stock (the administrator updates the stock)
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the cookies-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Users can add multiple cookies in multiple quantities in the cart at the  time of buying
* User can add comments (like emag/pcgarage) to the cookies
* Other users can view the comments, and can thumbs up/down if they think the comments are useful (this info will be visible to all users beside each comment)
