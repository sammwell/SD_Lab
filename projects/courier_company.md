...
Use Java or C# to design and implement a desktop application for a Courier Company. The application should have two types of users (a regular user - the courier - and an administrator user) which have to provide a username and a password in order to use the application.

The courier user can perform the following operations:
* CRUD on packages (including setting status: created, in transit, delivered)
* Search packages by different criteria (e.g., source, destination, size, urgency)

The administrator can perform the following operations:
* CRUD on transport cars
* CRUD on courier users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the packages delivered by couriers. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* There is a new type of user, the customer, that can create an account and send a package request.
  * He can also watch a given package id, and get notifications when its status changes
* The courier can create his daily delivery list (only undelivered items), and as it delivers items, they get changed to "delivered"
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the package-related and car-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Administrator can change customer's subscription which limits the number of products he can post in one day. The subscriptions can be added/changed by the admin.
* The user has a virtual wallet which he can use to change/upgrade his subscription, or, pay for extra individual packages to be sent.
