...
Use Java or C# to design and implement a desktop application for Airline Tickets. The application should have three types of users (a regular user, and employee of an airline company and an administrator user) which have to provide a username and a password in order to use the application.
...
* Generate two types of report files, one in pdf format and one in txt or html format, with the companies & current flights. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* Buy a single-person ticket from a flight with his money. For now, only the admin can set the wallet amount.
* The user can add/remove a company to/from his watch list
* The user gets a live notification when a new flight is added on the company he is watching
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the tickets-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* The user can buy multiple tickets on the same flight
* The employee specifies how many tickets are available for a flight to limit the amount one user can buy
* The user can now view his previously purchases
* The price of a ticket increases multiple times once it reaches certain availability thresholds (e.g., when only 2 remaining, the price increases 20% from original price, when only 5 remaining the price increases 10%).
