...
Use Java or C# to design and implement a desktop application for Gym Training & Monitoring. The application should have two types of users (a regular user - the trainer - and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Create a training program from a list of activities
* Create a diet program from a list of food items

The administrator can perform the following operations:
* CRUD on activity types (name, duration, intensity, etc)
* CRUD on food items (name, duration, calories count, etc)
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the training programs and diet programs created by each trainer. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* Trainers can now create accounts for the customers (customers are associated to that specific trainer who created them)
* The customer can:
  * Login
  * View his attendance. For every day he logs in, the attendance is completed automatically for that day.
* The trainer gets a live notification when a new customer associated to him logs in the application.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the training programs, and diet-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* The customers can view the currently associated trainer and rate it (between 1-5 stars).
* The list of trainers can be viewed by every customer (together with their ratings). If a customer wants, he can request transfer to another trainer. In this situation, both the source and target trainers need to agree on the transfer. Upon both confirming the transfer, the change is completed.
* The customer can request a new diet from his trainer, mentioning in a short description what he needs. The trainer can respond to that by sending a new diet.
