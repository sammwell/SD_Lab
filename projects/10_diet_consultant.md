...
Use Java or C# to design and implement a desktop application named Diet Consultant. The application should have two types of users (a regular user - the dietician - and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Create diet from list of foods for specific customer
* CRUD on customers

The administrator can perform the following operations:
* CRUD on food products (name, type, calories, glycemic index, etc).
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the diets with their components. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* A new type of user exists: the customer, which can do the following:
  * Login
  * Request a new diet (not to a specific dietician, but to the "system") with a given purpose. He can view the list of diets requested and which of them are pending or answered.
* All logged-in dieticians get a live notification when a new diet is requested by a customer, so they know to serve it.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the diets & food-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* The customer can:
  * See the list of available dieticians.
  * See which one answered his request.
  * Write a review together with a rating (between 1-5 stars) on the diet and dietician
* Administrator can change customer's subscription which limits the number of diets requests he can do in one day. The subscriptions can be added/changed by the admin
