...
Use Java or C# to design and implement a desktop application for the Fitness activity. The application should have two types of users (a regular user - the trainer - and an administrator user) which have to provide a username and a password in order to use the application.
...
New features:
* The trainer can allocate subscriptions for customers. When there is less than 2 days in the subscription, an automatic email will be sent to the customer. The customer will be able to respond YES or NO, simulating a payment and renewal.
* The trainer also gets a live notification if the employee responded to the email with YES, and the subscription will be automatically updated.
...
* The administrator can backup and later restore the app data.
