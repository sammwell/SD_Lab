...
Use Java or C# to design and implement a desktop application for Food Delivery. The application should have three types of users (a regular user, delivery user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Register an account
* View the list of restaurants and their items
* Order a single item from the list with virtual money from their wallet.

The administrator can perform the following operations:
* CRUD on restaurants and their items
* Edit regular user's wallet amount
* Generate two types of report files, one in pdf format and one in txt or html format, with the restaurants and items. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The user can add multiple items to an order
* The delivery user gets a live notification when a new order is placed. He can then change the order status from "created" to "in progress", and then finally in "delivered".
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the restaurants/items controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Regular Users can rate once the restaurants (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
* Regular users can add/remove restaurants from their favourites list
* Customers will get a discount based on the number of items bought from a specific restaurant. This is configured by the administrator dynamically (e.g., 5+ items 2%, 10+ items 4%)
