...
Use Java or C# to design and implement a desktop application for a Police Case Investigation Tracker. The application should have two types of users (a regular user - the cop/investigator and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* View the list of all cases, and search by different conditions (e.g., case type, urgency, involved entities). He can only see the ones he's been allocated to
* Can set the case status: created, in progress, blocked, completed

The administrator can perform the following operations:
* CRUD on cases and allocated cops to them
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the cases and allocated cops to each of them. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The cops can add new involved persons to the case
* The other cops that are logged in get a live notification when a new evidence is added to a case which they work on
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the case-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* A cop can watch/unwatch cases, which will limit the notifications he gets on case status/evidence change
* A cop can add evidence to a case, in the form of text or files (image/text file)
* A simple algorithm can suggest/predict the person(s) involved in the case that are most likely to be the culprits. It will take data from existing cases and also some risk factors (age, location, etc)
