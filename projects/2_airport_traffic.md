...
Use Java or C# to design and implement a desktop application for Airport Traffic Simulation. The application should have two types of users (a regular user - airport employee - and an administrator user) which have to provide a username and a password in order to use the application.
...
* Generate two types of report files, one in pdf format and one in txt or html format, with the companies and schedules. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* A non-logged-in user can see the timetable live.
* When the schedule "ticks in", the employee gets a live notification to approve/delay/reject the take off of that flight. If it delays/reject it, it will mark the flight as delayed/rejected in the timetable.
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the flight-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Customers can now create an account and reserve tickets on a flight
  * They can also rate once (between 1-5 stars) the flight. The rating will be visible to all other users (both logged in or not)
* If a flight is canceled that the user has reserved ticket, the customer will get a cancellation email.
