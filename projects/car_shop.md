...
Use Java or C# to design and implement a desktop application for Car Shop. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Search cars by properties (type, price, most bought, etc)
* Buy one car at a time with his money. For now, only the admin can set the wallet amount.

The administrator can perform the following operations:
* CRUD on cars
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the items in stock and the sold ones. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* Users can add/remove cars to/from their favourites list
* Users can rate once the cars (between 1-5 stars) and also search by rating. The admin user can edit the ratings.
* An user gets a live notification when a favourite car is back in stock (the administrator updates the stock)
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the cars-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* The cars have images besides their description
* User can add comments (like emag/pcgarage) to the cars
...
* When adding the car to the cart, the user must configure the car (e.g., bluetooth, AC, rearview camera, blind-spot monitoring, autopilot)
