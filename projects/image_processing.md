...
Use Java or C# to design and implement a desktop application for Image Processing. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Add pictures to the application, and see the processing output
* The user can choose from three types of image processing at the beginning: black & white, color inversion and sepia
* Delete pictures he added

The administrator can perform the following operations:
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the the processed images of each user (initial and processed target). The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* The user can add images to the queue to be processed and he can see the status of processing (queued, in progress, done)
* An user gets a live notification when a processing has been completed (if it takes too short time, you can add some fake delay to simulate a longer processing time on the server)
* An additional set of operations can be done against the images: resize to specific size, rotate left, rotate right
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the image-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Administrator can define available subscriptions which specify the amount of images can be processed per hour and the price for that subscription.
* Users can upgrade their default subscription (one image per hour) to another one. For this, he will use his virtual wallet, which can be managed by the administrator.
* The administrator can add new types of operations that combine the basic ones, which will trigger a different order of image processing (e.g., Type 1 = black/white + resize to X/Y, type 2 = black/white + rotate left)
